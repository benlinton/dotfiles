...
- Changing Claude Code permission rules (allow/ask/deny for Bash commands, mode-conditional gating, per-project policy): follow [`docs/claude-permission-gating.md`](docs/claude-permission-gating.md) — the native `settings.json` rules and the `permission-gate.sh` hook must be kept in sync. Most *per-project* changes are a `luma-policy set` and touch no file in this repo; that policy lives in `~/.config/luma` and is deliberately unmanaged and never committed
- `dot_local/bin/executable_luma-policy` and `dot_claude/executable_permission-gate.sh` duplicate the project-slug and repo-root logic on purpose, so the hook stays a single dependency-free file on the hot path. Change one, change both, then run `sh tests/permission-gate-test.sh`
